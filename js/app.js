...
function init() {
    ...
    navCorrecao.addEventListener('click', () => showPage(pageCorrecao, 'Correção'));
    navUsuarios.addEventListener('click', () => showPage(pageUsuarios, 'Gerenciamento de Usuários'));
...
function adjustCamera() {
    if (cameraStream && qrVideo) {
        ...
        if (videoContainer) {
            ...
            const aspectRatio = window.innerWidth > window.innerHeight ? 16/9 : 4/3;
    ...
    USERS.forEach(user => {
        ...
        deleteBtn.addEventListener('click', () => {
            if (confirm(`Deseja excluir o usuário ${user.name}?`)) {
                ...
                if (user.type === 'administrador' && USERS.filter(u => u.type === 'administrador').length <= 1) {
...
function resetCorrection() {
    ...
    scanInstruction.textContent = 'Posicione o QR code com os dados do aluno na câmera';
...
function goToNextStep() {
    if (currentCorrection.step === 1 && currentCorrection.aluno) {
        ...
        scanInstruction.textContent = 'Posicione o QR code com as respostas do gabarito na câmera';
...
function completeCorrection() {
    if (currentCorrection.step === 2 && currentCorrection.aluno && currentCorrection.gabarito) {
...
function processAlunoQRData(data) {
    try {
        ...
        document.getElementById('turma-turno').textContent = data.turma?.turno || 'N/A';
    ...
    if (currentCorrection.step === 1) {
        ...
        canvas.toBlob(async function(blob) {
            ...
            try {
                ...
                const response = await fetch('https://gerador-gabarito-leitor-gabarito.lh6c5d.easypanel.host/api/leitor/', {
                    method: 'POST',
                    body: formData
                });
